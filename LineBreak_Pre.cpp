...
LineBreak_Pre::LineBreak_Pre(vector<string> *t,int width):_width(width)
{
	m_pTexts=t;
}

LineBreak_Pre::~LineBreak_Pre()
{

}

bool LineBreak_Pre::check(int width)
{
	int maxwidth=-1;
...
bool LineBreak_Pre::setWidth(int width)
{
		...
		while(texts[i].size()!=0)
		{
			...
			else
			{
				...
				for(pos=width;pos>=0;pos--)
					if(texts[i][pos]==' ')break;
				m_pTexts->push_back( texts[i].substr(0,pos+1) );
				texts[i]=texts[i].substr(pos+1,texts[i].size()-pos);
			}
		}
	}
	return true;
